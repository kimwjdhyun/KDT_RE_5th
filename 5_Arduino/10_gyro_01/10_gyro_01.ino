...
void loop() {
  ...
  Wire.endTransmission(false); // 0x3B라는 레지스터 연결 유지(stop 조건)
...
/*
자이로 센서는 하나의 센서 값을 2바이트 (16비트)로 전송
하지만 아두이노 I2C 통신은 한 번에 1바이트만 통신이 가능
그래서 1바이트씩 두 번 값을 읽어와서 두 바이트를 합쳐서 저장

<< : 비트를 왼쪽으로 이동시키는 연산자
ex) x << n : x의 비트를 왼쪽으로 n칸 이동시키고, 오른쪽 빈 비트에는 0을 채움
  00010010 << 2 => 01001000 이렇게 바뀜

| : 비트를 합치는 연산자(or 연산자 이면서 비트를 조립할 때 이렇게 사용하기도 함)
*/
// 먼저 받아온 8비트를 왼쪽으로 8칸 옮기고, 다음 받아온 8비트를 합치는 코드
...
/*
  ...
  데이터를 읽어오는 지점을 지정했고,
  해당 지점부터 xyz축 가속도, 센서온도, 자이로스코프 값이 순서대로 오기 때문
